...
class CheetahEnv(DFlexEnv):
    ...
    def __init__(self, render=False, device='cuda:0', num_envs=4096, seed=0, episode_length=1000, no_grad=True, stochastic_init=False, MM_caching_frequency = 1, early_termination = False, env_params=None):
    ...
    def stepx(self, obs, actions):
        ...
        state_copy.joint_q = torch.cat([torch.zeros((num_envs,1)), obs[:, :8]], dim=-1).reshape(-1)
        state_copy.joint_qd = obs[:,8:].reshape(-1)
    ...
    def reward(self, state, action):
        progress_reward = state[:, 8]
        reward = progress_reward + torch.sum(action ** 2, dim = -1) * self.action_penalty
        return reward
