...
from env_utils import load_utils as lu
from env_utils import torch_utils as tu
import ipdb

class HopperEnv(DFlexEnv):

    def __init__(self, render=False, device='cuda:0', num_envs=4096, seed=0, episode_length=1000, no_grad=True, stochastic_init=False, MM_caching_frequency = 1, early_termination = True, env_params=None):
        num_obs = 11
        num_act = 3
        num_envs = num_envs*(2*num_obs+2*num_act+1)
        ...
        if env_params is None:
                self.env_params = {'density'   : 1000.0,
                                    'stiffness' : 0.0,
                                    'damping'   : 2.0,
                                    'contact_ke': 2.e+4,
                                    'contact_kd': 1.e+3,
    ...
    def init_sim(self):
        ...
        for i in range(self.num_environments):
            ...
            lu.parse_mjcf(os.path.join(asset_folder, "hopper.xml"), self.builder,
                density=self.env_params['density'],#1000.0,
                stiffness=self.env_params['stiffness'],#0.0,
                damping=self.env_params['damping'],#1.0,
                contact_ke=self.env_params['contact_ke'],#2.e+4,
                contact_kd=self.env_params['contact_kd'],#1.e+3,
                contact_kf=self.env_params['contact_kf'],#1.e+3,
                contact_mu=self.env_params['contact_mu'],#1.,
                limit_ke=self.env_params['limit_ke'],#1.e+3,
                limit_kd=self.env_params['limit_kd'],#1.e+1,
                armature=self.env_params['armature'],#0.1,
    ...
    def step(self, actions):
        numpy_out = False
        if actions.dtype==np.float32 or action.dtype==np.float64:
            actions = torch.tensor(actions).to(self.obs_buf)#self.device)
            numpy_out = True
        actions = torch.stack([actions]*self.num_envs, dim=0)
        ...
        if numpy_out:
            return self.obs_buf.detach().cpu().numpy()[0], self.rew_buf[0].item(), self.reset_buf[0].item(), self.extras
        return self.obs_buf[0], self.rew_buf[0], self.reset_buf[0], self.extras

    def stepx(self, obs, actions):
        num_envs = obs.shape[0]
        state_copy = self.copy_state()
        state_copy.joint_q = torch.cat([torch.zeros((num_envs,1)), obs[:, :5]], dim=-1).reshape(-1)
        state_copy.joint_qd = obs[:,5:].reshape(-1)
        actions = actions.view((num_envs, self.num_actions))

        actions = torch.clip(actions, -1., 1.)

        state_copy.joint_act = torch.cat([torch.zeros((num_envs, 3)), actions * self.action_strength], dim=-1)
        
        state_copy = self.integrator.forward(self.model, state_copy, self.sim_dt, self.sim_substeps, self.MM_caching_frequency)

        obs_buf = self.calculateObservationsx(state_copy, num_envs)
        rew_buf = self.reward(obs_buf, actions)

        return obs_buf, rew_buf, self.reset_buf, self.extras
    ...
    def reset(self, env_ids = None, force_reset = True):
        ...
        return self.obs_buf.detach().cpu().numpy()[0]
    ...
    def reward(self, obs_buf, action):
        height_diff = obs_buf[:, 0] - (self.termination_height + self.termination_height_tolerance)
        height_reward = torch.clip(height_diff, -1.0, 0.3)
        height_reward = torch.where(height_reward < 0.0, -200.0 * height_reward * height_reward, height_reward)
        height_reward = torch.where(height_reward > 0.0, self.height_rew_scale * height_reward, height_reward)
        angle_reward = 1. * (-obs_buf[:, 1] ** 2 / (self.termination_angle ** 2) + 1.)
        progress_reward = obs_buf[:, 5]
        rew_buf = progress_reward + height_reward + angle_reward + torch.sum(action ** 2, dim = -1) * self.action_penalty
        return rew_buf

    def calculateObservationsx(self, state, num_envs):
        obs_buf = torch.cat([state.joint_q.view(num_envs, -1)[:, 1:], state.joint_qd.view(num_envs, -1)], dim = -1)
        return obs_buf
